/*
StackState API

This API documentation page describes the StackState server API. The StackState UI and CLI use the StackState server API to configure and query StackState.  You can use the API for similar purposes.  Each request sent to the StackState server API must be authenticated using one of the available authentication methods.   *Note that the StackState receiver API, used to send topology, telemetry and traces to StackState, is not described on this page and requires a different authentication method.*  For more information on StackState, refer to the [StackState documentation](https://docs.stackstate.com).

API version: 5.2.0
Contact: info@stackstate.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package stackstate_api

import (
	"encoding/json"
)

// NotificationChannelNotFound struct for NotificationChannelNotFound
type NotificationChannelNotFound struct {
	Message   string `json:"message"`
	ChannelId string `json:"channelId"`
	Type      string `json:"_type"`
}

// NewNotificationChannelNotFound instantiates a new NotificationChannelNotFound object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewNotificationChannelNotFound(message string, channelId string, type_ string) *NotificationChannelNotFound {
	this := NotificationChannelNotFound{}
	this.Message = message
	this.ChannelId = channelId
	this.Type = type_
	return &this
}

// NewNotificationChannelNotFoundWithDefaults instantiates a new NotificationChannelNotFound object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewNotificationChannelNotFoundWithDefaults() *NotificationChannelNotFound {
	this := NotificationChannelNotFound{}
	return &this
}

// GetMessage returns the Message field value
func (o *NotificationChannelNotFound) GetMessage() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Message
}

// GetMessageOk returns a tuple with the Message field value
// and a boolean to check if the value has been set.
func (o *NotificationChannelNotFound) GetMessageOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Message, true
}

// SetMessage sets field value
func (o *NotificationChannelNotFound) SetMessage(v string) {
	o.Message = v
}

// GetChannelId returns the ChannelId field value
func (o *NotificationChannelNotFound) GetChannelId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ChannelId
}

// GetChannelIdOk returns a tuple with the ChannelId field value
// and a boolean to check if the value has been set.
func (o *NotificationChannelNotFound) GetChannelIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ChannelId, true
}

// SetChannelId sets field value
func (o *NotificationChannelNotFound) SetChannelId(v string) {
	o.ChannelId = v
}

// GetType returns the Type field value
func (o *NotificationChannelNotFound) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *NotificationChannelNotFound) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *NotificationChannelNotFound) SetType(v string) {
	o.Type = v
}

func (o NotificationChannelNotFound) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["message"] = o.Message
	}
	if true {
		toSerialize["channelId"] = o.ChannelId
	}
	if true {
		toSerialize["_type"] = o.Type
	}
	return json.Marshal(toSerialize)
}

type NullableNotificationChannelNotFound struct {
	value *NotificationChannelNotFound
	isSet bool
}

func (v NullableNotificationChannelNotFound) Get() *NotificationChannelNotFound {
	return v.value
}

func (v *NullableNotificationChannelNotFound) Set(val *NotificationChannelNotFound) {
	v.value = val
	v.isSet = true
}

func (v NullableNotificationChannelNotFound) IsSet() bool {
	return v.isSet
}

func (v *NullableNotificationChannelNotFound) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNotificationChannelNotFound(val *NotificationChannelNotFound) *NullableNotificationChannelNotFound {
	return &NullableNotificationChannelNotFound{value: val, isSet: true}
}

func (v NullableNotificationChannelNotFound) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNotificationChannelNotFound) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
